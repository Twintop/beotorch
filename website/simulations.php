...
$pageTitle = "Simulation Browser";
include_once $_SERVER['DOCUMENT_ROOT'] . '/includes/header.inc.php';

if (isset($_GET['s']) && strlen($_GET['s']) > 0)
{
	$_POST['specialization'] = $_GET['s'];
}
...
if (isset($_GET['ft']) && strlen($_GET['ft']) > 0)
{
	$_POST['fightType'] = $_GET['ft'];
}
?>
			...
			<div class="panel panel-default">
				<div class="panel-heading"><a data-toggle="collapse" href="#SearchFilterSimulations">Search or Filter  Simulations</a></div>
				...
				?>" style="margin-top: 10px;">
					<form action="simulations.php" method="post" name="simulations_form" class="form-horizontal">                            
					<?php
					
					$simCounts = SimulationCountsGet($mysqli, null);    

					$classes = $simCounts[0];
					$specializations = $simCounts[1];
					$servers = $simCounts[2];
					$simulationTypes = $simCounts[3];
					$iterations = $simCounts[4];
					$statuses = $simCounts[5];
					$itemLevels = $simCounts[6];
					...
					?>           
						...
						<div class="form-group col-xs-4">
							<label class="control-label col-xs-5" for="none"></label>
							<div class="col-xs-7 sliders_buffer" style="height: 38px; line-height: 38px;">
								&nbsp;
							</div>
						</div>
						<div class="form-group col-xs-4">
						...
						?>  /> 
						<div class="form-group col-xs-4">
							<label class="control-label col-xs-5" for="iterations">Item Level:</label>
							<div class="col-xs-7 sliders_buffer" style="height: 76px; line-height: 38px;">
			    	...
			    	$x = 0;
			    	$increment = round($maxIterations / 3, -3);
			    	...
			    	$increment = round(($maxItemlevel - $itemLevels[0]['ItemLevel']) / 3, -1);
			...
			if (isset($_POST['class']) || isset($_GET['c']))
			{
				$slClass = $_POST['class'];
			}
			else
			...
			if (isset($_POST['specialization']) || isset($_GET['s']))
			{
			...
			$simulations = $SimulationRepository->SimulationList(-2, $slStatus, 500, 0, 2, $slCharacter, $slFightType, $slClass, $slSpecialization, $slScaleFactors, $slReportArchived, $slIterationsLow, $slIterationsHigh, $slServer, $slItemlevelsLow, $slItemlevelsHigh, $slBossCountLow, $slBossCountHigh);
            ...
            {
            	echo "<p>No simulations match your filters!</p>";
            }
